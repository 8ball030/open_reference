...
def create_roles(apps, schema_editor):
    ...
    for organisation_role in organisation_roles:
        # we first need to create the role and then add the organisation
        if not Role.objects.filter(name=organisation_role[1]).exists():
            Role.objects.create(
                name=organisation_role[1],
                description=organisation_role[2],
            )
        else:
            # we merge the tool
